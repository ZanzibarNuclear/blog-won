  ...
  <div class="grid grid-cols-6 px-4 min-w-96">
    <header class="app-header fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-10">
      <div class="max-w-7xl mx-auto mt-4 px-4 py-3 flex items-center relative">
        ...
        <h1
          class="absolute left-1/2 -translate-x-1/2 text-x3l font-bold text-[nuclear] bg-[#f6f2eb]/30 backdrop-blur-sm rounded-lg p-1"
        >
          World of Nuclear
        </h1>
      </div>
        ...
        <UModal v-model="isOpen">
          <UCard>
            <div
              ...
              :key="item._path"
              class="mx-5 p-2 hover:bg-blue-200 rounded-md"
            >
              {{ item.icon }}
